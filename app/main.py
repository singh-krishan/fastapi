...
@app.get("/")
def read_root():
    return {"message": "welcome to the world of apis"}
try:
...
@app.post("/posts")
def create_posts(post: Post, email: EmailStr = Depends(oauth2.get_current_user)):
...
def login(user: UserCreate):
    ...
    if user == None:
        raise HTTPException(status_code=404, detail="no record found")
    if not pwd_context.verify(user.password, storeduser['password']):
